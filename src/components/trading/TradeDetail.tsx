...
export default function TradeDetail({ trade, onEditTrade }: TradeDetailProps) {
  ...
  const [deleteOpened, { open: openDeleteModal, close: closeDeleteModal }] =
    useDisclosure(false);
  const [requestedTrades, setRequestedTrades] = useState<Set<string>>(
    new Set()
  );
    ...
    <Container size="lg" className="py-8 relative">
      ...
      >
        <Stack gap="md">
          ...
          {/* Looking For Section */}
          <Box>
            <Title order={isMobile ? 4 : 3} mb="sm">
              Looking For
            </Title>
                ...
                >
                  <Stack align="center" gap={4}>
                    <TradeDragonFaceCard
                      ...
                      size={isMobile ? "sm" : "md"}
                    />
                    <Text fw={600} size={isMobile ? "sm" : "md"} ta="center">
                      {item.dragon.name}
                    </Text>
          ...
          <Box mt="xl" pt="md">
            <Group justify="center" gap="xs" mb="sm">
              ...
              <Text size="sm" c="dimmed">
                Trade powered by <span className="font-medium">Dragon City Helper</span>
              </Text>
            </Group>
            <Text
              size="xs"
            ...
            >
              Connect with more trainers • Complete your collection
            </Text>
          </Box>
          ...
          {session?.user?.id !== trade.userId ? (
            <Group justify="flex-end" mt={{ base: "md", sm: "xl" }}>
              <Button
                ...
                fullWidth={isMobile}
                leftSection={
                  isRequested ? <IconMessageCircle size={20} /> : null
                }
              >
                    ...
                    <Card key={request.id} withBorder p="md" radius="md">
                      <Group justify="space-between">
                        <Group gap="sm">
                          ...
                          <div>
                            <Text fw={500}>
                              {request.createdBy.name || "Anonymous User"}
                            </Text>
                            <Text size="sm" c="dimmed">
                              {formatDistanceToNow(
                        ...
                        <Stack gap="xs">
                          {request.createdBy.Contacts?.inGameName && (
                            <Group gap="sm">
                              <IconUser size={18} />
                              <Text>
                                In-game: {request.createdBy.Contacts.inGameName}
                              </Text>
                            </Group>
                          )}
                          {request.createdBy.Contacts?.discord && (
                            <Group gap="sm">
                              <IconMessage size={18} />
                              <Text>
                                Discord: {request.createdBy.Contacts.discord}
                              </Text>
                            </Group>
                          )}
                          {request.createdBy.Contacts?.email && (
                            <Group gap="sm">
                              <IconMail size={18} />
                              <Text>
                                Email: {request.createdBy.Contacts.email}
                              </Text>
                            </Group>
                          )}
                          {request.createdBy.Contacts?.phone && (
                            <Group gap="sm">
                              <IconPhone size={18} />
                              <Text>
                                Phone: {request.createdBy.Contacts.phone}
                              </Text>
                            </Group>
                          )}
                        </Stack>
